...
NODATATITLE = _("Not enough sales in the period")
...
class open_sales_series(models.TransientModel):
    ...
    forecast_type_abs = fields.Selection(
        [("monetary", "Monetary"), ("asbolute", "Absolute Quantity"),],
        string="Forecast Base",
        required=True,
        default="monetary",
        help="""* Monetary - in default company currency
        * Absolute - in product default units of measurement (without transformations)"""
    )
    product_ids = fields.Many2many("product.product", string="Products")
    template_ids = fields.Many2many("product.template", string="Product Templates")
    ...
    partner_ids = fields.Many2many(
        "res.partner",
        "res_partner_forecast_rel_table",
        "res_partner_id",
        "sale_forecast_id",
        string="Partners",
    )
    ...
    def _calculate_data(self):
        ...
        interval = self.interval
        freq = interval[0]
        if self.forecast_type_abs == "asbolute":
            forecast_select = """
